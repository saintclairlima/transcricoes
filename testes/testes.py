...
if __name__ == '__main__':
    models = {
        ...
        'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2':'MiniLM-L12-v2',
        'jvanhoof/all-MiniLM-L6-multilingual-v2-en-es-pt-pt-br-v2': 'MiniLM-L6-en-es-pt-pt-br-v2'
    }

    labels = []
    original = [0,45,90,146,203,241]
    tiles = []
    for model in models:
        t1 = time.time()
        ...
        print("%s score: %.4f" %(labels[-1], tiling_score(original, tl)) + f' - {int(time.time() - t1)} segundos')

    plot_tiles([original]+tiles, ["Original"]+labels)
    with open('doc.txt', 'r', encoding='utf-8') as arq:
        d = arq.read().split(' ')
