...
class FragmentadorSemantico:
    def __init__(self, nome_modelo:str, dispositivo:str=None):
        ...
        if nome_modelo == 'bertscore':
            ...
            self.get_similarity_scores = self._get_similarity_scores_bertscore
        else:
            ...
            self.get_similarity_scores = self._get_similarity_scores_transformer

    def _get_similarity_scores_bertscore(self, vec_a:str, vec_b:str):
        """
    ...
    def _get_similarity_scores_transformer(self, vec_a:str, vec_b:str):
        """
        This function calculates the similarity scores between pairs of text chunks using the specified model.
        It supports several types of models, including BERT, sequence matcher, Jaccard index, and transformers from HuggingFace.
        ...
        for i in range(len(vec_a)):
            a_tokens = self.tokenizer(vec_a[i], padding=True, truncation=True, max_length=256, return_tensors='pt').to(self.dispositivo)
            b_tokens = self.tokenizer(vec_b[i], padding=True, truncation=True, max_length=256, return_tensors='pt').to(self.dispositivo)

            with torch.no_grad():
                embeddings_a.append(self.modelo(**a_tokens).last_hidden_state.mean(dim=1))
                embeddings_b.append(self.modelo(**b_tokens).last_hidden_state.mean(dim=1))
    ...
    def create_similarity_graph(self, sentences, K):
        """
        ...
        a, b = zip(*couples)
        similarities = self.get_similarity_scores(a, b)
    ...
    def create_tiles(self, text:str) -> Tuple[List[int], List[str]]:
        """
        ...
        graph = self.create_similarity_graph(sentences, 5)
        ...
        tiles = self.compact_clusters(tiles)
    ...
    def fragmentar(self, texto:str) -> List[str]:
        _, fragmentos =  self.create_tiles(text=texto)
        return fragmentos